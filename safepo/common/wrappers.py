    ...
    def __init__(
        ...
        for agent in range(self.num_agents):
            self.share_observation_spaces[f"agent_{agent}"] = Box(
                low=-10, high=10, shape=(self.share_obs_size,)
            )
            self.observation_spaces[f"agent_{agent}"] = Box(
                low=-10, high=10, shape=(self.obs_size,)
            )
...
class MultiFormationNavEnv():
    """多机器人编队与导航环境封装。

    与 ``MultiGoalEnv`` 类似，但可以在构造时指定机器人数量 ``num_agents``，
    方便通过命令行参数控制编队规模。
    """
    ...
    def __init__(
        ...
        formation_shape: str = "mesh",
        formation_line_axis: str = "x",
        formation_wedge_half_angle_deg: float = 35.0,
        formation_desired_direction: list | np.ndarray | None = None,
        formation_target_distance: float | None = None,
        formation_completion_error_threshold: float | None = None,
        formation_completion_reward: float | None = None,
    ):
        """Args:
            task: Safety-Gymnasium 多智能体任务 ID。
            seed: 随机种子。
            num_agents: 机器人（智能体）数量，可由命令行参数指定。
            render_mode: 渲染模式，'human' 用于显示画面，None 则不渲染。
            formation_*: 与 RMP / ``cfg_train`` 一致的编队形状与尺度，写入底层任务奖励。
        """
        assert num_agents >= 1, "num_agents must be >= 1"
        if formation_desired_direction is None:
            formation_desired_direction = [0.0, 1.0]
        fd = np.asarray(formation_desired_direction, dtype=np.float64).reshape(2)
        make_kwargs: dict = {
            "formation_shape": str(formation_shape).strip().lower(),
            "formation_line_axis": str(formation_line_axis).strip().lower(),
            "formation_wedge_half_angle_deg": float(formation_wedge_half_angle_deg),
            "formation_desired_direction": [float(fd[0]), float(fd[1])],
        }
        if formation_target_distance is not None:
            make_kwargs["formation_target_distance"] = float(formation_target_distance)
        if formation_completion_error_threshold is not None:
            make_kwargs["formation_completion_error_threshold"] = float(
                formation_completion_error_threshold,
            )
        if formation_completion_reward is not None:
            make_kwargs["formation_completion_reward"] = float(formation_completion_reward)
        ...
        # 使用底层环境的第一个 agent 的动作空间作为统一动作空间定义
        self.single_action_space = self.env.action_space('agent_0')

        self.num_agents = num_agents
        base_agents = list(self.env.possible_agents)
        assert len(base_agents) == self.num_agents, (
            f"Underlying env has {len(base_agents)} agents "
            f"but num_agents={self.num_agents} was requested."
        )
        self._formation_agents = base_agents

        self.action_spaces = {
            agent: self.env.action_space(agent) for agent in self._formation_agents
        }
        ...
        self.share_observation_spaces = {}
        self.observation_spaces = {}
        for agent in self._formation_agents:
    ...
    def __getattr__(self, name: str) -> Any:
        """透传未知属性到底层 env，禁止访问私有属性。"""
        if name.startswith('_'):
            raise AttributeError(f"accessing private attribute '{name}' is prohibited")
        return getattr(self.env, name)

    def _get_obs(self):
        """为编队中的每个机器人构造带有 one-hot ID 的局部观测，并做标准化。"""
        state = self.env.task.obs()
        obs_n = []
        for a in range(self.num_agents):
            agent_id_feats = np.zeros(self.num_agents, dtype=np.float32)
    ...
    def reset(self, seed=None):
        """重置环境，返回 (obs, share_obs, avail_actions)。"""
        self.env.reset(seed=seed)
        return self._get_obs(), self._get_share_obs(), self._get_avail_actions()
    ...
    def step(
        ...
        """执行一步编队导航交互，兼容上层 MAPPO/MA 算法接口。"""
        dict_actions = {}
        # actions 约定为按索引排列的张量/数组，这里只取前 num_agents 个
        for agent_id, agent in enumerate(self._formation_agents):
            dict_actions[agent] = actions[agent_id].cpu().numpy()
...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            # Safety-Gymnasium (and MultiFormationNavEnv): render() takes no mode; mode comes from make(render_mode=...).
            frame = env.render()
            remote.send(frame if frame is not None else True)
        elif cmd == 'close':
            env.close()
            remote.close()
            break
...
class ShareSubprocVecEnv(ShareVecEnv):
    ...
    def render(self, mode='human'):
        """Show the first sub-env only (only rank-0 is created with ``render_mode`` for formation tasks)."""
    ...
    def close_extras(self):
        """Close worker processes and communication pipes."""
...
class ShareDummyVecEnv(ShareVecEnv):
    ...
    def close_extras(self):
        """Close all in-process environments."""
    ...
    def render(self, mode='human'):
        del mode  # underlying env uses render_mode fixed at construction
