...
from safepo.common.wrappers import (
    ...
    MultiGoalEnv,
    MultiFormationNavEnv,
)
from safepo.common.mult_cm import MultiNavAtacom
...
def make_cm_multi_goal_env(task, seed, cfg_train):
    """
    Creates and returns a multi-agent environment using MuJoCo scenarios.
...
def make_formation_nav_env(task, seed, num_agents, cfg_train, render_mode=None):
    """
    Creates and returns a multi-agent formation navigation environment.

    The underlying task is a Safety-Gymnasium multi-agent task. The number of
    robots (agents) in the formation is controlled by ``num_agents``.

    Args:
        task: Safety-Gymnasium task id.
        seed: Random seed.
        num_agents: Number of robots in the formation.
        cfg_train: Training configuration, used for vec-env creation.
        render_mode: Render mode, 'human' to display, None to disable.

    Returns:
        env: A multi-agent vectorized formation navigation environment.
    """

    def get_env_fn(rank):
        def init_env():
            """
            Initializes and returns a MultiFormationNavEnv instance for the given rank.
            """
            # 只在第一个环境启用渲染（避免多个窗口）
            env_render_mode = render_mode if rank == 0 else None
            fd = cfg_train.get("formation_desired_direction", [0.0, 1.0])
            ftd = cfg_train.get("formation_target_distance", 0.5)
            env = MultiFormationNavEnv(
                task=task,
                seed=seed + rank * 1000,
                num_agents=num_agents,
                render_mode=env_render_mode,
                formation_shape=cfg_train.get("formation_shape", "mesh"),
                formation_line_axis=cfg_train.get("formation_line_axis", "x"),
                formation_wedge_half_angle_deg=float(
                    cfg_train.get("formation_wedge_half_angle_deg", 35.0),
                ),
                formation_desired_direction=fd,
                formation_target_distance=float(ftd),
                formation_completion_error_threshold=cfg_train.get(
                    "formation_completion_error_threshold",
                ),
                formation_completion_reward=cfg_train.get(
                    "formation_completion_reward",
                ),
            )
            return env
