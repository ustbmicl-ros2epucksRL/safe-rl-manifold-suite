...
from safepo.common.env_jrpc import RemoteEnvJRPC


class SingleNavAtacomJRPC(AtacomEnvWrapper):
    def __init__(
        self,
        base_env,
        gamma=0.99,
        ...
        for obs in self.env.env_obstacles():
            if obs["name"] == "hazards":
                num = obs["items"].shape[0]
                c_hazard = StateConstraint(
                    dim_q=dim_q,
                    dim_out=num,
                    fun=self.hazard_f,
                    jac_q=self.hazard_J,
                    ...
                    slack_beta=slack_beta_hazard_constraint,
    ...
    def _get_q(self):
        pos = self.env.agent_pose()
        theta = self.env.agent_angle()
    ...
    def J_drive(self, q):
        theta = q[2]
        return np.array([[np.cos(theta), 0], [np.sin(theta), 0], [0.0, 1.0]])
    ...
    def step(self, action):
        ...
        # print(f"action_space.low: {self.action_space.low}")
        # print(f"action_space.high: {self.action_space.high}")
    ...
    def step_action_function(self, alpha):
        ...
        dq_err = -self.K_c * (
            Jc_inv @ self.constraints.c(self.q, self.x, origin_constr=False)
        )
...
if __name__ == "__main__":
    from env_jrpc import RemoteEnvJRPC

    base_env = RemoteEnvJRPC(8)
    mdp = SingleNavAtacomJRPC(base_env=base_env, timestep=0.2)
    while True:
        mdp.reset()
        for i in range(100):
            # action = np.random.uniform(mdp.info.action_space.low, mdp.info.action_space.high)
            action = np.ones_like([1.0, 0.0])
            # action[0] = 1.
            (_, next_obs, reward, cost, terminated, truncated, info) = mdp.step(action)
            print(next_obs)
            if truncated:
                print("############ Collisions! ####################")
                break
