...
from safepo.common.env import make_ma_mujoco_env, make_ma_isaac_env, make_ma_multi_goal_env, make_formation_nav_env
from safepo.multi_agent.rmp_corrector import RMPCorrector
from safepo.common.popart import PopArt
from safepo.common.model import (
    MultiAgentActor as Actor,
    MultiAgentCritic as Critic,
    save_multi_agent_actor_traced_jit,
)
...
class Runner:
    ...
    def __init__(self,
        ...
        torch.autograd.set_detect_anomaly(bool(self.config.get("detect_anomaly", False)))
        ...
        if use_tensorboard:
            tb_dir = os.path.abspath(os.path.join(config["log_dir"], "tb"))
            print(f"TensorBoard logging enabled. View with: tensorboard --logdir={tb_dir}")
        ...
        self.buffer = []
        for agent_id in range(self.num_agents):
            tr = PPO_Trainer(config, self.policy[agent_id])
            share_observation_space = self.envs.share_observation_space[agent_id]
        ...
        # 初始化 RMP 修正器
        self.rmp_corrector = RMPCorrector(
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.config["episode_length"]):
                ...
                # 渲染环境（如果启用了渲染）
                if self.config.get('render', False) or getattr(self.envs, 'render_mode', None) == 'human':
                    self.envs.render()
            ...
            if episode % self.config["eval_interval"] == 0 and self.config["use_eval"]:
                eval_rewards, eval_costs = self.eval()[:2]
            ...
            if len(done_episodes_rewards) != 0:
                ...
                # If no episode is finished, keep logger cache bounded.
    ...
    def _get_follower_action(self, agent_id, n_envs):
        """为 follower agent 生成固定策略动作（随机动作）"""
        action_space = self.envs.action_space[agent_id]
        if hasattr(action_space, 'sample'):
            # 如果是 gymnasium 空间，使用 sample 方法
            actions = []
            for _ in range(n_envs):
                action = action_space.sample()
                actions.append(torch.from_numpy(action).to(self.config["device"]))
            return torch.stack(actions)
        else:
            # 否则生成零动作
            action_shape = action_space.shape[0] if hasattr(action_space, 'shape') else 2
            return torch.zeros(n_envs, action_shape, device=self.config["device"])
    ...
    def collect(self, step):
        ...
        n_envs = self.config["n_rollout_threads"]
        
        for agent_id in range(self.num_agents):
            self.trainer[agent_id].prep_rollout()
            value, action, action_log_prob, rnn_state, rnn_state_critic = self.policy[agent_id].get_actions(
        ...
        if self.rmp_corrector.rmp_enabled:
            try:
                ...
                # 如果 RMP 修正失败，使用原始动作
                if step % 100 == 0:  # 只偶尔打印警告，避免日志过多
                    print(f"Warning: RMP correction failed at step {step}: {e}. Using original actions.")
    ...
    def insert(self, data):
        ...
        # 只为 leader 插入数据到 buffer
        if 'Frank'in self.config['env_name']:
            obs_to_insert = obs[0]
        else:
            obs_to_insert = obs[:, 0]
        
        self.buffer[0].insert(share_obs[:, 0], obs_to_insert, rnn_states[:, 0],
                         rnn_states_critic[:, 0], actions[0],
                         action_log_probs[0],
                         values[:, 0], rewards[:, 0], masks[:, 0], None,
                         active_masks[:, 0], None)

    def train(self):
        for agent_id in torch.randperm(self.num_agents):
            action_dim = self.buffer[agent_id].actions.shape[-1]
            factor = torch.ones(self.config["episode_length"], self.config["n_rollout_threads"], 1, device=self.config["device"])
    ...
    def save(self):
        ...
        for agent_id in range(self.num_agents):
            ...
            actor.eval()
            torch.save(actor.state_dict(), str(self.save_dir) + "/actor_agent" + str(agent_id) + ".pt")
            torch.save(self.trainer[agent_id].policy.critic.state_dict(), str(self.save_dir) + "/critic_agent" + str(agent_id) + ".pt")

            save_multi_agent_actor_traced_jit(
                actor,
                self.policy[agent_id].obs_space,
                str(self.save_dir) + "/actor_agent" + str(agent_id) + "_traced.pt",
                device,
                self.config,
                agent_id,
            )
    ...
    def restore(self):
        for agent_id in range(self.num_agents):
            self.policy[agent_id].actor.load_state_dict(torch.load(str(self.model_dir) + '/actor_agent' + str(agent_id) + '.pt'))
            self.policy[agent_id].critic.load_state_dict(torch.load(str(self.model_dir) + '/critic_agent' + str(agent_id) + '.pt'))
        ...
        while True:
            ...
            for agent_id in range(self.num_agents):
                ...
                if 'Frank'in self.config['env_name']:
                    ...
                    obs_to_eval = eval_obs[:, agent_id]
                eval_actions, temp_rnn_state = self.policy[agent_id].act(obs_to_eval,
            ...
            for eval_i in range(self.config["n_eval_rollout_threads"]):
                if eval_dones_env[eval_i]:
                    ...
                    success = goal_met and (ep_cost == 0)
                    eval_episode_successes.append(success)

            if eval_episode >= eval_episodes:
                success_count = sum(eval_episode_successes)
                return np.mean(eval_episode_rewards), np.mean(eval_episode_costs), success_count

    @torch.no_grad()
    def compute(self):
        for agent_id in range(self.num_agents):
            self.trainer[agent_id].prep_rollout()
            next_value = self.policy[agent_id].get_values(self.buffer[agent_id].share_obs[-1],
...
if __name__ == '__main__':
    set_np_formatting()
    args, cfg_env, cfg_train = multi_agent_args(algo="mappo_rmp")
