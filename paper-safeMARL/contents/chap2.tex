...
经典扩展卡尔曼滤波器（EKF）通常假设噪声统计特性已知且固定，但在实际运行条件变化时该假设往往不成立。近年来的学习型方法通过深度模型弥补这一不足：例如深度 IMU 偏置估计~\cite{buchanan2022learning_ekf}用于视觉惯性里程计中的偏置学习，TLIO~\cite{liu2023denoise_imu}利用卷积网络实现紧耦合惯性里程计，KalmanNet~\cite{revach2022kalmannet}通过端到端方式学习卡尔曼增益，而 A-KIT~\cite{levy2024akit}则利用 Transformer 估计自适应过程噪声。本文提出的基于 CNN 的噪声自适应模块参数规模较小（约 5K），专门用于从 IMU 时间窗口中学习与速度相关的测量协方差，从而提供自适应的 $R_t$，提升下游安全滤波器的状态估计精度。
