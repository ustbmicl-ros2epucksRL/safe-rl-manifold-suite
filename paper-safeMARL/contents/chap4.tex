...
实验场景如图~\ref{fig:rmp_env} 所示，为大小为 $3 \times 3$ 的二维平面区域，其中在左右两侧设置静态墙体障碍物（\texttt{Sigwalls}），其中心位置分别为 $(-1.0,\,0), \quad (1.0,\,0)$,从而在环境中形成沿 $y$ 轴方向延展的狭窄通道结构。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.4\linewidth]{images/training_reward.png}
    \includegraphics[width=0.4\linewidth]{images/training_cost.png}
    \caption{训练过程中的reward 与 cost。}
    \label{fig:train_result}
\end{figure}
通过三次20轮次的任务评估，实验结果如表~\ref{tab:line_formation_result} 所示。可以看出，所提出的 GCPL 方法在所有指标上均取得了最优表现，其累计回报（$11.51 \pm 7.54$）、成功率（$77.00\% \pm 6.00\%$）以及编队完成度（$91.00\% \pm 2.00\%$）均显著优于其他基线方法，同时具有最低的安全代价（$10.53 \pm 0.57$）。该结果表明，引入几何约束能够有效提升多智能体策略的结构一致性与任务稳定性。

相比之下，纯强化学习方法 mappo 与 mappolag 表现较差，不仅回报较低，且安全代价较高，说明在缺乏显式约束的情况下，策略难以兼顾安全性与协同控制目标。mappo\_CBF 通过引入控制屏障函数在一定程度上降低了安全代价，但其编队完成度与成功率仍明显落后于 GCPL，表明仅依赖安全约束不足以保证编队结构的有效形成。RMPflow 虽然具有一定的几何一致性，但由于缺乏全局优化机制，其整体性能仍然有限。
\begin{table}[htbp]
    \centering
    \caption{线形编队实验结果}
    ...
    RMPflow    & $3.71 \pm 6.24$    & $32.85 \pm 19.37$  & $58.00 \pm 5.00$ & $13.00 \pm 11.00$ \\
    GCPL       & $11.51 \pm 7.54$   & $10.53 \pm 5.57$   & $81.00 \pm 2.00$ & $67.00 \pm 6.00$ \\
    mappo\_CBF & $7.73 \pm 1.35$    & $25.17 \pm 2.25$   & $35.00 \pm 3.00$ & $30.00 \pm 4.00$ \\
    ...
    \begin{figure*}[t]
    \centering
    \subfloat[reward]{\includegraphics[height=4cm]{images/rmp_reward.png}}
    \hfill
    \subfloat[cost]{\includegraphics[height=4cm]{images/rmp_cost.png}}
