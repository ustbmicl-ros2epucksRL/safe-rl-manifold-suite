...
该力在机器人接近安全边界时趋于无穷大，形成软几何屏障，从力学原理上保证无碰撞。
...
\begin{itemize}
    ...
    \item 消融实验：对比仅采用 mappo 强化学习方法和融合方法在任务中的表现。同时对比关键超参数对模型性能产生的的影响。
\end{itemize}

采用的基线方法包括 mappo、happo、macpo、mappo\_lag。
...
为了全面评估算法性能，本文采用以下指标：
1）累计奖励（Total Reward）
统计策略在一个完整 episode 中获得的累计奖励：$R = \sum_t r_t$，该指标反映策略整体行为质量。
2）安全代价（Collision Cost）
统计系统运行过程中产生的累计安全代价：$C = \sum_t c_t$，
该指标用于评估系统安全性。
3）任务成功率（Success Rate）
成功率定义为：
\[SR = \frac{N_{success}}{N_{total}}\]
其中：$N_{success}$ 为成功到达目标且未发生碰撞的 episode 数；$N_{total}$ 为总测试 episode 数
该指标反映策略在复杂环境中的任务完成能力。
...
同时，我们对训练结果进行了评估，分别用 MAPPO 和我们的方法完成 20 轮次的任务，结果同样显示出我们的方法 reward 更高和 cost 更低的结果。
\begin{figure*}[t]
\centering
