...
\section{问题定义}
为刻画机器人安全导航中的安全关系，考虑一个机器人在包含 $n$ 个障碍物的复杂环境中进行导航任务。令 $s_t = [p_t^\top, v_t^\top]^\top \in \mathcal{S} \subseteq \mathbb{R}^d$ 表示机器人在离散时间步 $t$ 时的状态，其中 $p_t \in \mathbb{R}^{d_p}$ 代表位置向量，$v_t \in \mathbb{R}^{d_p}$ 代表速度向量。为了定量描述安全，引入基于距离的约束函数。安全性的核心要求是机器人与环境中任何障碍物之间必须保持一定的物理间隔。定义约束集合如下：
\begin{equation}
    c_i(s_t) = d_{\text{safe}} - \|p_t - o_i\|_2 \leq 0, \quad \forall i \in \{1, \ldots, n\}
\end{equation}

其中，$d_{\text{safe}} > 0$ 表示预设的最小安全距离阈值，$o_i \in \mathbb{R}^{d_p}$ 为第 $i$ 个障碍物在空间中的坐标，$\|\cdot\|_2$ 为欧几里得范数。

若状态 $s_t$ 满足对所有 $i$ 均有 $c_i(s_t) \leq 0$，则称该状态是安全的。在物理上意味着机器人的中心与所有障碍物的中心的距离均大于等于 $d_{\text{safe}}$。在实际应用中，该距离考虑了机器人自身的几何半径以及感知系统的不确定性冗余。
...
\end{equation}
\begin{equation}
    \text{s.t. } c_i(s_t) \leq 0, \quad \forall t \in \{0, \dots, T\}, \forall i \in \{1, \dots, n\}
\end{equation}

与传统的强化学习不同，CMDP 要求算法在探索过程中不仅要学习任务技能，还必须严格遵守状态空间的边界，避免任何形式的碰撞。

此外，考虑到机器人系统（如差速驱动轮式机器人或无人机）普遍存在非完整约束，不能简单地假设状态可以任意改变。因此，本文将机器人的底层运动学建模为连续时间的控制仿射系统：
\begin{equation}
    \dot{s} = f(s) + G(s) a
\end{equation}

在该模型中：$a \in \mathcal{A} \subseteq \mathbb{R}^m$ 是 $m$ 维控制控制量（如推力、转矩或速度指令）；$f(s): \mathcal{S} \rightarrow \mathbb{R}^d$ 称为漂移项，代表系统在无控制输入时的固有动力学演化；$G(s): \mathcal{S} \rightarrow \mathbb{R}^{d \times m}$ 为控制输入矩阵，定义了控制动作如何映射到状态的变化率上。动力学特例：质点模型（Single-Integrator）： 若 $f(s) = 0$ 且 $G(s) = I$，系统简化为全向移动模型。差速驱动模型： $G(s)$ 包含三角函数项（如 $\cos\theta, \sin\theta$），用以体现机器人朝向与运动速度之间的耦合关系。通过结合上述动力学模型与 CMDP 框架，本文的研究重点在于如何在复杂的动力学约束下，保证机器人在轨迹执行过程中的实时安全性。

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{images/framework (1).png}
    \figcap{基于约束流形过滤的安全强化学习方法框架}{Framework of the constrained manifold policy learning method.}
    \label{fig:placeholder}
\end{figure}

\section{基于约束流形过滤的安全强化学习方法}
...
为缓解感知噪声对安全决策的影响，本文构建基于数据驱动的扩展卡尔曼滤波（EKF）框架，通过卷积神经网络学习观测噪声参数，使测量协方差能够随运动状态自适应调整，从而在速度相关噪声条件下维持较高的状态估计精度，保证安全过滤器输入的可靠性。近年来，神经网络辅助卡尔曼滤波与自适应测量噪声估计已在部分已知动力学和惯性导航任务中得到应用~\cite{revach2022kalmannet,buchanan2023learning_ekf,cohen2025akit}，为本文的数据驱动状态估计模块提供了方法依据。
\end{itemize}
...
\subsubsection{（1）约束函数与活动约束}
本文参考 ATACOM~\cite{liu2022atacom,liu2023safe_manipulation} 的零空间投影思想，并结合 CBF 与预测式安全过滤器中“执行前修正动作”的处理范式~\cite{ames2017cbf_qp,wabersich2021predictive_safety_filter}，将策略输出的名义动作分解为约束切空间内的自由运动和指向安全集合内部的修正项。为避免引入过多与实现无关的增广变量，本文直接从不等式约束出发。设系统安全集合为
\begin{equation}
    \mathcal{C}=\{s\in\mathcal{S}\mid c_i(s)\leq 0,\ i=1,\ldots,k\},
    \label{eq:safe_set}
\end{equation}
其中 $c_i(s)=0$ 表示第 $i$ 个安全约束的边界，$c_i(s)<0$ 表示状态位于安全侧。

在每个控制时刻，安全过滤器只选择接近边界或已经被触发的约束构成活动约束集合 $\mathcal{I}_a(s)$，并将对应的约束函数写为
\begin{equation}
    c_a(s)=\bigl[c_i(s)\bigr]_{i\in\mathcal{I}_a(s)}\in\mathbb{R}^{r},
    \quad
    J_s(s)=\frac{\partial c_a(s)}{\partial s}.
    \label{eq:active_constraints}
\end{equation}
这样处理后，安全过滤器只在可能发生约束违背的方向上修正动作，而不会对远离边界的安全运动施加不必要的限制。

\subsubsection{（2）零空间投影}
设$a_{\text{unsafe}} \in \mathbb{R}^m$表示由强化学习策略输出的未受约束的动作。
考虑控制仿射形式的系统动力学
\[
    \dot{s}=f(s)+G(s)a .
\]
对活动约束函数取时间导数可得：
\begin{equation}
    \dot{c}_a(s)=J_s(s)f(s)+J_c(s)a,
    \quad
    J_c(s)=J_s(s)G(s)\in\mathbb{R}^{r\times m}.
    \label{eq:constraint_deriv}
\end{equation}

设 $J_c^\dagger$ 为矩阵 $J_c$ 的 Moore--Penrose 伪逆，定义零空间投影算子：
\begin{equation}
    N_c(s)=I_m-J_c^\dagger(s)J_c(s).
\end{equation}
该投影矩阵将名义动作投影到活动约束的切空间，使其不再增加活动约束的一阶变化。安全动作计算为
\begin{equation}
    a_{\text{safe}}
    =
    N_c(s)a_{\text{unsafe}}
    -
    J_c^\dagger(s)
    \bigl(J_s(s)f(s)+K_c c_a(s)\bigr),
    \label{eq:safe_action}
\end{equation}
其中 $K_c\in\mathbb{R}^{r\times r}$ 为正定对角增益。公式~\eqref{eq:safe_action} 的第一项保留了策略动作中不破坏约束的一部分，第二项补偿系统漂移动力学并将活动约束维持在安全边界以内。对于本文点机器人速度控制实验，$f(s)$ 可由低层运动模型吸收或近似为零，此时实现形式可简化为 $a_{\text{safe}}=N_c a_{\text{unsafe}}-J_c^\dagger K_c c_a(s)$。

\begin{theorem}[活动约束不变性]
\label{thm:safety}
假设活动约束雅可比矩阵 $J_c(s)$ 在系统轨迹上具有满行秩，且当前可能触碰边界的约束均已包含在 $\mathcal{I}_a(s)$ 中。若初始活动约束满足 $c_a(s_0)\leq 0$，并且控制输入按照公式~\eqref{eq:safe_action} 计算，则在连续时间下活动约束不会从安全侧穿过边界。
\end{theorem}

\begin{proof}
\renewcommand{\qedsymbol}{}%
由于 $J_c$ 满行秩，有 $J_cJ_c^\dagger=I_r$ 且 $J_cN_c=0$。将控制律~\eqref{eq:safe_action} 代入约束导数公式~\eqref{eq:constraint_deriv} 可得：
\begin{align*}
    \dot{c}_a
    &= J_s f(s)+J_c\!\left[N_c a_{\text{unsafe}}-J_c^\dagger\bigl(J_s f(s)+K_c c_a\bigr)\right] \\
    &= J_s f(s)-J_cJ_c^\dagger J_s f(s)-J_cJ_c^\dagger K_c c_a \\
    &= -K_c c_a .
\end{align*}
因此 $c_a(t)=\exp(-K_ct)c_a(0)$。若 $c_a(0)\leq 0$，则对任意 $t\geq0$ 都有 $c_a(t)\leq0$，活动约束不会越过安全边界。
\end{proof}

\textbf{说明（具体实施）.} 在实际实现中，为了提高数值稳定性，本文采用阻尼伪逆（damped pseudoinverse）
\[
    \hat{J}_c^\dagger = J_c^\top (J_c J_c^\top + \epsilon I_r)^{-1}
\]

其中 $\epsilon > 0$ 为较小的正数。此时会引入残差 $\|J_c\hat{J}_c^\dagger-I_r\|=O(\epsilon)$。

因此理论上的严格不变性会变为带有小残差的近似不变性，即约束误差保持在 $O(\epsilon)$ 的邻域内。同时修正项 $\hat{J}_c^\dagger K_c c_a$ 会持续抑制活动约束误差，从而在 $\epsilon$ 足够小时保持较低的约束违反程度。
...
\begin{equation}
    \begin{split}
        &R_{\alpha}(s_t,a_{\text{unsafe},t},a_{\text{safe},t}) \\
        &=R(s_t,a_{\text{safe},t})-\alpha P(a_{\text{unsafe},t},a_{\text{safe},t})
    \end{split}
\end{equation}

其中，$\alpha$ 是惩罚权重，$P$ 是基于修正幅度的惩罚函数，$a_{\text{unsafe},t}$ 是策略给出的名义动作，$a_{\text{safe},t}$ 是经过安全过滤后实际应用于系统的动作。

\begin{algorithm}
\caption{基于约束流形过滤的安全强化学习方法（CMPL）}
\label{alg:cmpl}
\begin{algorithmic}[1]
\STATE \textbf{输入：} 约束函数 $c(s)$，安全过滤参数 $K_c$，奖励校准权重 $\alpha$
\FOR{$i=1$ \TO $N$}
    \STATE 初始化可行状态 $s_0$
    \FOR{$k=0$ \TO $T-1$}
        \STATE 采样名义动作 $a_{\text{unsafe},k} \sim \pi(\cdot \mid s_k)$
        \STATE 观测状态 $s_k$
        \STATE 选择活动约束 $c_a(s_k)$，计算 $J_c(s_k)$ 与 $N_c(s_k)$
        \STATE 根据公式~\eqref{eq:safe_action} 得到安全动作 $a_{\text{safe},k}$
        \STATE 执行 $a_{\text{safe},k}$
        \STATE 观测下一时刻状态 $s_{k+1}$ 及奖励 $r_k$
        \STATE 计算校准奖励 $\tilde{r}_k=r_k-\alpha P(a_{\text{unsafe},k},a_{\text{safe},k})$
        \STATE 将样本 $(s_k,a_{\text{unsafe},k},a_{\text{safe},k},s_{k+1},\tilde{r}_k)$ 提供给强化学习更新
    \ENDFOR
\ENDFOR
\end{algorithmic}
\end{algorithm}
...
\subsection{安全可达性分析}
约束流形过滤器需要已知障碍物的位置，本文假设这些信息可以通过静态障碍物的先验地图获得，例如来自离线建图或仿真中的真实环境数据。
为了进一步刻画状态和动作对安全性的影响，本文引入可达性分析思想，通过可行性值函数描述机器人从当前状态出发是否存在满足硬安全约束的控制策略。该思路源于 Hamilton–Jacobi（HJ）可达性分析~\cite{bansal2017hj_reachability,bansal2021deepreach}：标准 HJ 可达性以\textbf{无折扣的 minimax 形式}刻画安全可达集，理论上需在线求解动态规划，计算代价较高。为便于离线、数据驱动地近似可行性值函数，本文采用其\textbf{折扣可达性}（discounted reachability）形式，即引入折扣因子 $\gamma$ 构造折扣可行贝尔曼算子（见式~\eqref{eq:feasible_bellman}），并基于已采集轨迹拟合该值函数；这一折扣近似与可达性约束强化学习中常用的安全贝尔曼方程一致~\cite{ganai2023iterative_reachability,hsu2024safety_filter}。由此得到的离线可行性值函数用于降低部署阶段的在线计算压力，而非声称求解了精确的 HJ 可达集。
与传统基于期望累计代价的软约束方法不同，本文关注的是状态级硬约束，即要求机器人在未来轨迹的所有时刻均满足
\begin{equation}
    c(s_t)\leq 0,\quad \forall t\in\mathbb{N}.
\end{equation}

\subsubsection{（1）可行性值函数}

给定策略 $\pi$，定义其对应的可行性状态值函数为
\begin{equation}
    V_c^\pi(s)
    =
    \max_{t\in\mathbb{N}} c(s_t),
    \quad
    s_0=s,\ a_t\sim\pi(\cdot|s_t).
    \label{eq:vc_pi}
\end{equation}

该函数表示从状态 $s$ 出发并按照策略 $\pi$ 执行时，整条未来轨迹中最大的约束违反程度。若 $V_c^\pi(s)\leq 0$ ,则说明未来所有状态均满足 $c(s_t)\leq 0,\quad \forall t\in\mathbb{N}$,
即策略 $\pi$ 能够从状态 $s$ 出发持续满足硬安全约束。若 $V_c^\pi(s)>0$，则说明未来轨迹中至少存在某一时刻发生约束违反。

进一步在所有策略中选择未来最大约束违反程度最小的策略，可以得到最优可行性状态值函数
\begin{equation}
    V_c^*(s)
    =
    \min_{\pi}V_c^\pi(s)
...
该函数表示从状态 $s$ 出发时，在所有可能策略中所能达到的最小未来最大约束违反程度。若 $V_c^*(s)\leq 0$，则说明至少存在一种策略能够使机器人未来全程满足安全约束，因此状态 $s$ 为可行状态；若 $V_c^*(s)>0$，则说明即便选择最安全策略，未来仍然会出现约束违反，因此状态 $s$ 为不可行状态。
...
其中，$Q_c^*(s,a)$ 表示在状态 $s$ 执行动作 $a$ 后，未来轨迹中能够达到的最小最大约束违反程度。与 $V_c^*(s)$ 相比，$Q_c^*(s,a)$ 固定了第一步动作，从而用于判断某个具体动作是否可能导致未来进入危险区域。

\subsubsection{（2）离线近似}

上述可行性值函数直接依赖未来整条轨迹。若要精确计算 $V_c^*$ 和 $Q_c^*$，通常需要从任意状态出发与环境交互。
然而，在离线设置中，机器人只能利用已经采集的轨迹数据。因此，本文借助贝尔曼递推思想，构造如下折扣可行贝尔曼算子，将当前状态--动作对的未来风险分解为当前状态风险和后继状态未来风险：
\begin{align}
    \mathcal{T}^*Q_c(s,a)
    &=
    (1-\gamma)c(s)
    +
    \gamma\max\{c(s),V_c^*(s')\}, \\
    V_c^*(s')&=\min_{a'}Q_c(s',a').
    \label{eq:feasible_bellman}
\end{align}

其中，$\mathcal{T}^*$ 表示最优可行贝尔曼算子，$\gamma\in(0,1)$ 为折扣因子。本文的安全可行性关注未来轨迹中的最大约束违反程度，因此递推核心为$\max\{c(s),V_c(s')\}$，
该项表示当前状态风险 $c(s)$ 与后继状态未来风险 $V_c(s')$ 中的较大者。

式~\eqref{eq:feasible_bellman} 中的 $V_c^*(s')=\min_{a'}Q_c(s',a')$
表示在后继状态 $s'$ 中选择未来风险最小的动作。由于本文处于离线学习设置，训练过程仅使用离线数据集 $D_{off}$中已经采集到的状态–动作样本。本文采用反向期望分位回归，在数据范围内近似低风险动作对应的状态值函数。
反向期望分位损失定义为
\begin{equation}
    L_{\mathrm{rev}}^\tau(u)
    =
    \left|\tau-\mathbf{I}(u>0)\right|u^2.
    \label{eq:rev_expectile}
\end{equation}
其中，$\tau\in(0.5,1)$，$u=Q_c(s,a)-V_c(s)$。
当 $Q_c(s,a)>V_c(s)$ 时，说明动作 $a$ 的风险高于当前状态基准，此时 $u>0$，有 $\mathbf{I}(u>0)=1$，损失权重为 $1-\tau$，由于 $\tau>0.5$，因此高风险动作的权重会削弱，反之低风险动作会被赋予更大的权重。因此，$V_c(s)$ 会被推向数据范围内较小的 $Q_c(s,a)$，即更安全的动作价值。由此可以近似得到
\begin{equation}
    V_c(s)\approx \min_{a\sim\mathcal{D}_{\mathrm{off}}}Q_c(s,a).
\end{equation}

在上述反向期望分位回归及贝尔曼递推的基础上，状态值函数 $V_c$ 和动作值函数 $Q_c$ 通过以下两个损失函数联合学习：
\begin{align}
    L_{V_c}
    &=
    \mathbb{E}_{(s,a)\sim\mathcal{D}_{\mathrm{off}}}
    \left[
    L_{\mathrm{rev}}^\tau
    \left(
    Q_c(s,a)-V_c(s)
    ...
    L_{Q_c}
    &=
    \mathbb{E}_{(s,a,s')\sim\mathcal{D}_{\mathrm{off}}}
    \left[
    \left(
    (1-\gamma)c(s)
    +
...
式~\eqref{eq:loss_vc} 用于学习状态 $s$ 下的低风险基准值。它通过反向期望分位损失对不同动作样本赋予不同权重，使 $V_c(s)$ 更接近离线数据中较低风险动作对应的 $Q_c(s,a)$。

式~\eqref{eq:loss_qc} 用于学习从状态--动作对 $(s,a)$ 出发的未来最大风险。其回归目标来自式~\eqref{eq:feasible_bellman} 所定义的可行贝尔曼递推，即同时考虑当前约束值 $c(s)$ 和后继状态未来风险 $V_c(s')$。因此，$L_{Q_c}$ 使 $Q_c(s,a)$ 能够估计执行动作 $a$ 后可能产生的未来最大安全风险。

通过联合优化式~\eqref{eq:loss_vc} 和式~\eqref{eq:loss_qc}，可以在不进行在线交互的条件下学习得到近似的安全可行性状态值函数 $V_c$ 和安全可行性动作值函数 $Q_c$。其中，$V_c(s)$ 用于判断状态是否处于可行区域，$Q_c(s,a)$ 用于判断具体动作是否可能导致未来进入约束违反区域。

\subsubsection{（3）与安全过滤器的集成}
在部署阶段，学习得到的 $V^*_c$ 用于调节安全过滤器的保守程度。我们定义一个平滑权重函数：
\begin{equation}
...
这种机制使得安全过滤器在部署阶段具备环境感知能力，而无需进行实时的障碍物检测或跟踪——预先计算得到的 $V^*_c$ 已经编码了来自先验地图的静态环境可达性结构。

\subsection{数据驱动的状态估计}
状态估计的精度会直接影响安全过滤器的判断结果。机器人在导航过程中通常依赖 GPS、视觉里程计等位置传感器获取状态信息，但这类测量并不稳定，噪声大小会随运行条件发生变化。以高速运动为例，机体振动、GPS 多路径效应以及视觉图像中的运动模糊都会更加明显，此时传感器观测误差往往高于低速运动状态。本文将这种与速度相关的测量不确定性表示为：
\begin{equation}
\sigma(v) = \sigma_{\text{base}}(1 + k |v|)
\label{eq:vel_noise}
\end{equation}

其中，$\sigma_{\text{base}}$ 为基础噪声标准差，$k > 0$ 表示速度缩放系数，$|v|$ 表示机器人当前运动速度。常规 EKF 一般采用固定测量噪声协方差 $R = \sigma_{\text{base}}^2 I$，这种设定在低速或噪声较稳定的情况下较为方便，但当机器人速度较高时，这种固定协方差模型往往会过度信任噪声较大的测量数据，从而降低状态估计精度。基于此，本文采用一种数据驱动的扩展卡尔曼滤波方法，利用最近时间段的 IMU 测量数据预测当前测量噪声协方差，使滤波过程能够随运动状态变化进行调整。该设计与学习辅助滤波方法~\cite{revach2022kalmannet,buchanan2023learning_ekf,cohen2025akit} 的思路一致，但本文仅学习测量噪声协方差，以保持滤波结构的可解释性和轻量化。
...
其中，$F_t = \frac{\partial f}{\partial s}\big|_{\hat{s}_{t-1}}$ 表示动力学函数对状态的雅可比矩阵，$Q_t$ 为过程噪声协方差。
...
为了避免手动设置的误差，本文通过一个卷积神经网络噪声参数适配器来进行学习。该网络以一段 IMU 测量的滑动窗口 作为输入：
\begin{equation}
    z_t = \text{CNN}\left(\{\omega^{\text{IMU}}_i, a^{\text{IMU}}_i\}_{i=t-L}^{t}\right)
\label{eq:noise_cnn}
\end{equation}

其中 $L=10$ 表示窗口长度（在 10 Hz 采样频率下对应 1 秒的数据）。噪声参数适配器是一个轻量级 CNN（约 5K 参数）：网络结构包含两层一维卷积层（通道数 $6 \rightarrow 32 \rightarrow 64$，卷积核大小为 3），每层后接 ReLU 激活函数；随后通过自适应平均池化（adaptive average pooling），并接两层全连接层（$64 \rightarrow 64 \rightarrow 2$），最终输出
\begin{equation}
...
\begin{figure}[!t]
    ...
    \figcap{不同任务场景图}{Illustrations of different task scenarios in Safety-Gymnasium.}
    \end{figure}
...
\begin{itemize}
    ...
    为评估完整框架在安全性与任务性能上的综合表现，本文在 Safety-Gymnasium ~\cite{ray2019benchmarking,ji2023safety_gymnasium,ji2024omnisafe} 的 Goal、Circle、Push 与 MultiGoal 任务中，与 PPO、PCPO、PPO-Lag、TRPO-Lag 及 MAPPO 系列方法进行对比，并进一步通过“流形过滤器—可达性预训练—数据驱动 EKF”逐步加入的消融实验，分析各模块对 Reward、Cost 与 Steps 的贡献。
...
此外，当改变机器人速度的大小与运动方向时，可以观察到安全域分布的显著变化。表明安全可达空间与机器人的运动状态的相关性。当智能体速度增大或朝向发生变化时，其在有限时间内的可达集合随之改变，安全域边界也会发生调整。
...
\begin{figure*}[thpb]
	...
	\figcap{安全空间分布图}{Distribution of the learned safe reachable space.}
...
\subsubsection{（1）基线方法}
本文在单智能体和多智能体场景中，与多种经典的安全强化学习算法进行对比，包括 PPO~\cite{schulman2017ppo}、 PCPO~\cite{yang2020projection} 、 PPO-Lag~\cite{stooke2020responsive} 、 TRPO-Lag~\cite{achiam2017constrained} ，多智能体实验包含了 MAPPO 、 HAPPO~\cite{kuba2022happo} 、 MACPO 以及 MAPPO-Lag。
...
\begin{table}[t]
\centering
\tabcap{实验超参数设置}{Experimental hyperparameter settings}
...
\begin{table*}[h!]
    \begin{center}
      \tabcap{Safety-Gymnasium 在不同任务和惩罚系数设置下的结果}{Results on Safety-Gymnasium under different tasks and collision-penalty settings}
...
验证每个组件的贡献。实验结果为 3 个随机种子、$10^5$ 训练步以及 50 个评估回合的平均值。完整系统在速度相关传感器噪声条件下运行（$\sigma_{\text{base}} = 0.1$m，$k=5$）。

表~\ref{tab:ablation} 给出各配置下的定量结果。前三行在无噪声条件下评估，后两行启用速度相关传感器噪声。
\begin{table}[t]
\centering
\tabcap{Goal 任务消融实验结果}{Ablation results on the Goal task}
\label{tab:ablation}
\setlength{\tabcolsep}{11pt}% 增大列间距
\begin{tabular}{@{}l|cc@{}}
\toprule
配置 & Reward & Cost \\
\midrule
PPO（baseline） & $-4.28 \pm 2.98$ & $50.58 \pm 32.63$ \\
+流形过滤器 & $\phantom{-}0.50 \pm 0.53$ & $\phantom{0}3.29 \pm 0.75$ \\
+自适应裕度（可达性） & $\phantom{-}1.12 \pm 0.51$ & $\mathbf{\phantom{0}0.54 \pm 0.94}$ \\
+EKF（完整系统，含噪声） & $-0.78 \pm 1.23$ & $\phantom{0}4.91 \pm 5.81$ \\
  \midrule
  完整系统（无奖励校准） & $\phantom{-}0.59 \pm 0.76$ & $\phantom{0}8.47 \pm 4.98$ \\
  \bottomrule
  \end{tabular}
  \end{table}
...
\figcap{消融实验训练曲线}{Training curves of ablation configurations.}
\label{fig:training_curves}
\end{figure}

图~\ref{fig:training_curves} 给出各配置的训练曲线。各组件作用如下：
\begin{itemize}
\item \textbf{流形过滤器}：主导贡献，cost 下降 94\%（$50.58 \rightarrow 3.29$），3 个种子稳定收敛于 $[2.62, 4.10]$ 区间。

\item \textbf{自适应裕度（可达性）}：以速度自适应的安全裕度作为单步 BRT 近似，在静态过滤器之上将 cost 进一步降至 $0.54$（额外下降 83\%），其中 2/3 种子达到完全零违规。该行为表中最低 cost 配置。

\item \textbf{完整系统（含 EKF 与噪声）}：启用速度相关传感器噪声后 cost 均值升至 $4.91$，仍较 baseline 下降 90\%。EKF 与学习到的噪声适配器使整套安全栈在感知不确定性下仍然有效。

\item \textbf{奖励校准}：完整系统中移除奖励校准后 cost 上升约 73\%（$4.91 \rightarrow 8.47$），表明奖励校准的必要性。
\end{itemize}
完整带噪系统相比无约束 PPO 基线实现了 90\% 的 cost 下降（$50.58 \rightarrow 4.91$），无噪场景下加入自适应裕度可达到约 99\% 的下降（$0.54$）。
...
\begin{table}[t]
\centering
\tabcap{Goal 任务 EKF 对比实验}{Comparative EKF experiments on the Goal task}
...
EKF（学习得到的 $R$） & $\mathbf{0.27 {\pm} 0.05}$ & $-7.87 {\pm} 0.94$ & $9.31 {\pm} 1.68$ \\
\bottomrule
\end{tabular}
\end{table}
...
需要说明的是，在这一独立评估中三种状态估计配置的 cost 差异并不显著（$9.14$、$9.58$、$9.31$，误差区间相互重叠）：数据驱动 EKF 的主要贡献在于显著提升定位精度（位置误差 $0.54 \rightarrow 0.27$m），而本框架的安全收益主要来自可达性自适应裕度而非噪声协方差学习本身。这一点在消融实验（表~\ref{tab:ablation}）中得到印证：无噪场景下自适应裕度已将 cost 降至 $0.54$，含噪声的完整系统仍维持 $4.91$（较 baseline 下降 90\%）。更精确的状态估计通过为安全过滤器提供更可靠的输入间接支撑安全性，二者在完整系统中互补。

\textbf{计算开销。} 噪声参数适配器 CNN 带来的计算开销非常小：该网络约有 5K 个参数，在 CPU 上每一步推理时间小于 $0.1$,ms，相比环境步进时间（5ms）和 PPO 前向传播时间（1ms）几乎可以忽略不计。
...
\figcap{ROS2 集成架构}{ROS~2 integration architecture.}
\label{fig:ros2arch}
\end{figure}

为了验证安全层具有策略无关性（policy-agnostic），本文在 E-puck 机器人（直径 $\varnothing70mm$）上部署了基于距离的安全过滤器和标准 EKF（固定 $R$），并在 Webots 仿真环境中进行实验。实验场景为一个 $2\text{m} \times 2\text{m}$ 的区域，其中放置 6 个圆柱形障碍物，GPS 噪声为 $\sigma=0.04$m，IMU 航向角噪声为 $\sigma=0.05$,rad。名义控制策略刻意采用一个简单的 P 控制器而不是训练得到的强化学习策略，以证明该安全架构可以在无需重新训练的情况下迁移到新的机器人平台和控制器上。

在 Webots 实验中，安全过滤器采用基于距离的速度缩放机制（相比流形过滤器更为简单），同时使用具有固定噪声协方差的标准 EKF。实验结果如图~\ref{fig:ros}，在没有安全过滤的情况下，20 次实验中有 8 次发生碰撞（碰撞率 40\%），其中 4 次由于碰撞导致导航错误而未能到达目标（成功率 80\%）。而本文的方法实现了零碰撞，成功率达到 85\%。其中 3 次失败是由于机器人在密集障碍区域附近发生导航超时，但均在没有碰撞的情况下安全结束。值得注意的是，安全过滤器实际上提高了成功率（85\% 对比 80\%），因为未过滤的碰撞会破坏导航稳定性。EKF 通过融合里程计和 GPS 测量，将位置误差降低了 27.5\%（$0.051 \rightarrow 0.037$m）（见表~\ref{tab:webots}）。

\begin{table}[t]
    \centering
    \small
    \tabcap{Webots E-puck 实验验证}{Webots E-puck experimental validation}
    \label{tab:webots}
    \begin{tabular}{l@{\hspace{1em}}|@{\hspace{1em}}c c}
    \toprule
...
\begin{figure*}[t]
\centering
\subfloat[实验环境]{\includegraphics[height=4cm]{images/ros_setup.png}}
\hfill
\subfloat[轨迹对比]{\includegraphics[height=4cm]{images/trajectory.png}}
\figcap{Webots 中 E-puck 实验验证}{Webots validation on E-puck robots.}
\label{fig:ros}
\end{figure*}
