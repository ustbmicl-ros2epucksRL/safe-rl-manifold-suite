...
为了进一步刻画状态和动作对安全性的影响，本文引入基于 HJ 可达性分析（Hamilton–Jacobi reachability analysis）的安全可达性分析方法，通过可行性值函数描述机器人从当前状态出发是否存在满足硬安全约束的控制策略。可达性分析和运行时安全过滤已被广泛用于安全关键自治系统~\cite{bansal2017hj_reachability,bansal2021deepreach,ganai2023iterative_reachability,hsu2024safety_filter}，本文则将其转化为离线可行性值函数，用于降低部署阶段的在线计算压力。
...
虽然在这一独立评估中各方法的 cost 差异不大，但消融实验（表~\ref{tab:ablation}）展示了完整系统的优势：无噪场景下自适应裕度已将 cost 降至 $0.54$，含噪声的完整系统仍维持 $4.91$（较 baseline 下降 90\%）。
