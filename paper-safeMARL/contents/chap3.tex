...
需要特别指出的是，若移除奖励校准，智能体虽可获得更高 reward（$16.83$），但同时会产生显著更多的违规（$71.63$ 对比 $12.51$）。各配置下的定量结果见表~\ref{tab:ablation}。
\begin{table}[t]
\centering
...
\figcap{学习曲线体现各组件的贡献}{Training curves showing component contributions.}
...
\item \textbf{奖励校准（Reward Calibration）}：对安全性至关重要；若移除该机制，智能体会利用约束边界换取更高 reward，从而显著放大安全代价，与表~\ref{tab:ablation} 中末两行及上文分析一致。
\end{itemize}
