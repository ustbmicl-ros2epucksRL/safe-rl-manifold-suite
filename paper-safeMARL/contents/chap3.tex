...
\begin{figure}
    ...
    \figcap{约束流形策略学习方法框架}{Framework of the constrained manifold policy learning method.}
...
表示在后继状态 $s'$ 中选择未来风险最小的动作。因此，本文采用反向期望分位回归，在数据范围内近似低风险动作对应的状态值函数。
反向期望分位损失定义为
\begin{equation}
    L_{\mathrm{rev}}^\tau(u)
    =
...
式~\eqref{eq:loss_vc} 用于学习状态 $s$ 下的低风险基准值。它通过反向期望分位损失对不同动作样本赋予不同权重，使 $V_c(s)$ 更接近离线数据中较低风险动作对应的 $Q_c(s,a)$，而不是简单拟合所有动作风险的平均水平。因此，$L_{V_c}$ 的作用可以理解为在数据范围内近似状态 $s$ 下的最小未来风险。

式~\eqref{eq:loss_qc} 用于学习从状态--动作对 $(s,a)$ 出发的未来最大风险。其回归目标来自式~\eqref{eq:feasible_bellman} 所定义的可行贝尔曼递推，即同时考虑当前约束值 $c(s)$ 和后继状态未来风险 $V_c(s')$：
\begin{equation}
    (1-\gamma)c(s)
    +
    \gamma\max\{c(s),V_c(s')\}.
\end{equation}
其中，$\max\{c(s),V_c(s')\}$ 表示当前状态风险和后继状态未来风险中的较大者，符合可行性值函数对未来最大约束违反程度的定义。因此，$L_{Q_c}$ 使 $Q_c(s,a)$ 能够估计执行动作 $a$ 后可能产生的未来最大安全风险。
...
在部署阶段，学习得到的 $V^*_c$ 用于调节安全过滤器的边界情况。
