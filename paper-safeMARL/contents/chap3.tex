...
+可达性预训练 & $\phantom{-}0.14 \pm 1.11$ & $10.16 \pm 8.65$ \\
+EKF（完整系统，含噪声） & $-0.61 \pm 0.46$ & $\mathbf{\phantom{0}5.87 \pm 5.31}$ \\
  \midrule
  完整系统（无奖励校准） & $\phantom{-}0.05 \pm 0.44$ & $10.25 \pm 6.90$ \\
  \bottomrule
  \end{tabular}
  \end{table}
...
\item \textbf{可达性预训练}：为完整系统提供可行域初始化先验；单独加入时受种子敏感性影响，3 个种子的 cost 分别为 $\{2.58, 19.58, 8.32\}$，均值上升至 $10.16$。

\item \textbf{完整系统（含 EKF 与噪声）}：cost 均值降至 $5.87$，其中一个种子达到 $0.00$，表明完整安全栈在感知噪声下仍然有效。

\item \textbf{奖励校准}：完整系统中移除奖励校准后 cost 上升 74\%（$5.87 \rightarrow 10.25$），逐种子比较中 2/3 种子支持校准的必要性。
\end{itemize}
完整系统相比无约束 PPO 基线实现了 88\% 的 cost 下降（$50.58 \rightarrow 5.87$）。
...
虽然在这一独立评估中各方法的 cost 差异不大，但消融实验（表~\ref{tab:ablation}）展示了完整系统的优势：在含噪声的训练条件下，cost 从 $10.16$ 降至 $5.87$。
