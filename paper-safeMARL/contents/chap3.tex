...
然而，不等式约束在优化和控制律设计中往往难以直接处理，且难以定义稳定的切空间。为此，本研究引入非负松弛变量 $\mu \in \mathbb{R}^k_{\geq 0}$。通过将原始约束函数与松弛变量结合，可以将不等式约束转化为严格的等式约束形式：
\begin{equation}
...
其中 $\mathcal{D} = \mathcal{S} \times \mathbb{R}^k_{\geq 0}$ 代表由原始状态与松弛变量构成的增广状态空间。
...
\begin{algorithmic}[1]
\STATE \textbf{输入：} 约束函数 $c(s)$ 及相关参数
\FOR{$i=1$ \TO $N$}
    \STATE 初始化可行状态 $s_0$ 与松弛变量 $\mu_0$
    \FOR{$k=0$ \TO $T-1$}
        \STATE 采样动作 $\alpha_k \sim \pi(\cdot \mid s_k)$
        \STATE 观测状态 $s_k$
        \STATE 计算 $J_c(s_k,\mu_k)$、$N_c(s_k,\mu_k)$，并得到安全投影后的动作 $a_k$
        \STATE 执行动作 $a_k$
        \STATE 观测下一时刻状态 $s_{k+1}$ 及奖励 $r_k$
        \STATE 将样本 $(s_k, a_k, s_{k+1}, r_k)$ 提供给强化学习更新
    \ENDFOR
\ENDFOR
\end{algorithmic}
\end{algorithm}
...
% \begin{algorithm}
    ...
    % \label{alg:cmpl}
    % \begin{algorithmic}[1]
    % \STATE \textbf{输入：} 约束函数 $c(s)$，策略网络 $\pi_{\theta}$，价值网络 $V_{\phi}$，安全过滤参数 $K_c$，奖励校准权重 $\alpha$
    % \STATE \textbf{初始化：} 策略参数 $\theta$，价值函数参数 $\phi$，经验缓冲区 $\mathcal{B}$
    % \FOR{$i=1$ \TO $N$}
    %     \STATE 清空经验缓冲区 $\mathcal{B}$
    %     \STATE 初始化可行状态 $s_0$ 与松弛变量 $\mu_0$
    %     \FOR{$k=0$ \TO $T-1$}
    %         \STATE 根据当前策略采样名义动作 $a_{\text{unc},k} \sim \pi_{\theta}(\cdot \mid s_k)$
    %         \STATE 计算 $J_c(s_k,\mu_k)$ 与 $N_c(s_k,\mu_k)$
    %         \STATE 根据约束流形投影得到安全动作 $a_{\text{safe},k}$
    %         \STATE 执行 $a_{\text{safe},k}$，观测 $s_{k+1}$、原始奖励 $r_k$ 与约束代价 $c_k$
    %         \STATE 根据动作修正幅度得到校准奖励
    %         \[
    %         \tilde{r}_k = r_k - \alpha P(a_{\text{unc},k},a_{\text{safe},k})
    %         \]
    %         \STATE 存储样本 $(s_k,a_{\text{unc},k},a_{\text{safe},k},\tilde{r}_k,c_k,s_{k+1})$ 至 $\mathcal{B}$
    %         \STATE 更新松弛变量 $\mu_{k+1}$
    %     \ENDFOR
    %     \STATE 基于 $\mathcal{B}$ 计算折扣回报 $\hat{R}_k$ 与优势函数 $\hat{A}_k$
    %     \STATE 使用 PPO 目标函数更新策略网络 $\pi_{\theta}$
    %     \[
    ...
    %     \min \left(
    %     \rho_k(\theta)\hat{A}_k,
    %     \operatorname{clip}(\rho_k(\theta),1-\epsilon,1+\epsilon)\hat{A}_k
    %     \right)
    %     \right]
    %     \]
    %     \STATE 其中，
    %     \[
    %     \rho_k(\theta)=
    %     \frac{\pi_{\theta}(a_{\text{unc},k}\mid s_k)}
    %     {\pi_{\theta_{\text{old}}}(a_{\text{unc},k}\mid s_k)}
    %     \]
    %     \STATE 更新价值网络 $V_{\phi}$
    %     \[
    ...
    %     \mathbb{E}_{\mathcal{B}}
    %     \left[
    %     \left(V_{\phi}(s_k)-\hat{R}_k\right)^2
    %     \right]
    %     \]
    % \ENDFOR
    % \STATE \textbf{输出：} 训练后的策略网络 $\pi_{\theta}$ 与约束流形安全过滤器
    % \end{algorithmic}
    % \end{algorithm}

\subsection{安全可达性分析}
约束流形过滤器需要已知障碍物的位置，本文假设这些信息可以通过静态障碍物的先验地图获得，例如来自离线建图或仿真中的真实环境数据。
为了进一步刻画状态和动作对安全性的影响，本文引入基于 HJ 可达性分析（Hamilton–Jacobi reachability analysis）的安全可达性分析方法，通过可行性值函数描述机器人从当前状态出发是否存在满足硬安全约束的控制策略。
...
\subsubsection{基线方法}
本文在单智能体和多智能体场景中，与多种经典的安全强化学习算法进行对比，包括 PPO~\cite{schulman2017ppo}、 PCPO~\cite{yang2020projection} 、 PPO-Lag~\cite{stooke2020responsive} 、 TRPO-Lag~\cite{achiam2017cpo} ，多智能体实验包含了 MAPPO 、 HAPPO~\cite{kuba2022happo} 、 MACPO 以及 MAPPO-Lag。
...
\item \textbf{奖励校准}：完整系统中移除奖励校准后 cost 上升 74\%（$5.87 \rightarrow 10.25$），表明奖励校准的必要性。
\end{itemize}
