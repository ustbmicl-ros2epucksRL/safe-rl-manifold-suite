...
\begin{equation}
    ...
    \label{eq:viability_fixed_point}
\end{equation}
...
HJ 可达性分析在计算最优可行值函数需要通过与环境交互进行蒙特卡罗估计，这在离线设置中是不可行的。
因此本文采用基于 HJ 可达性思想的离线安全可达空间近似方法，首先利用采集得到的轨迹数据构造离线数据集 $\mathcal{D}=\{(s,a,s')\}$，再通过函数逼近方式学习状态及状态-动作对的安全可行性。
在前文式~\eqref{eq:viability_fixed_point} 中，HJ 可达性分析对应的可行性不动点刻画了当前约束风险与后继状态风险之间的关系。
直接求解上述非折扣不动点需要充分覆盖状态-动作空间，在离线数据条件下较难实现。为提高数值稳定性，本文引入折扣因子 $\gamma\in(0,1)$，构造如下折扣可行 Bellman 目标：
\begin{equation}
...
由于离线数据集 $\mathcal{D}$ 不能覆盖全部动作空间，若直接对 $\min_a V(f(s,a))$ 进行显式优化，容易选择到数据分布之外的动作，进而造成不可靠的安全估计。为避免这一问题，本文采用 expectile regression 对数据集中较安全的动作进行加权拟合，从而在不显式枚举动作空间的情况下近似 HJ-VI 中的 $\min_a$ 项：
\begin{align}
...
其中，$L^\tau_{\text{rev}}(u)=|\tau-\mathbf{1}(u>0)|u^2$，$\tau\in[0.5,1]$。该损失对具有较低安全风险的动作赋予更高权重，使学习得到的值函数更接近可行策略所对应的安全边界。
