...
\section{问题定义}
为刻画机器人安全导航中的安全关系，考虑一个机器人在包含 $n$ 个障碍物的复杂环境中进行导航任务。令 $s_t = [p_t^\top, v_t^\top]^\top \in \mathcal{S} \subseteq \mathbb{R}^d$ 表示机器人在离散时间步 $t$ 时的状态，其中 $p_t \in \mathbb{R}^{d_p}$ 代表位置向量，$v_t \in \mathbb{R}^{d_p}$ 代表速度向量。为了定量描述安全，引入基于距离的约束函数。安全性的核心要求是机器人与环境中任何障碍物之间必须保持一定的物理间隔。定义约束集合如下：$$c_i(s_t) = d_{\text{safe}} - \|p_t - o_i\|_2 \leq 0, \quad \forall i \in \{1, \ldots, n\}$$其中，$d_{\text{safe}} > 0$ 表示预设的最小安全距离阈值，$o_i \in \mathbb{R}^{d_p}$ 为第 $i$ 个障碍物在空间中的坐标，$\|\cdot\|_2$ 为欧几里得范数。
...
此外，考虑到机器人系统（如差速驱动轮式机器人或无人机）普遍存在非完整约束，不能简单地假设状态可以任意改变。因此，本文将机器人的底层运动学建模为连续时间的控制仿射系统：$$\dot{s} = f(s) + G(s) a$$
...
表示在后继状态 $s'$ 中选择未来风险最小的动作。由于本文处于离线学习设置，训练过程仅使用离线数据集 $D_off$中已经采集到的状态–动作样本。本文采用反向期望分位回归，在数据范围内近似低风险动作对应的状态值函数。
反向期望分位损失定义为
\begin{equation}
    L_{\mathrm{rev}}^\tau(u)
    =
