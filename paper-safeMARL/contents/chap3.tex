...
本文构建了一种面向安全强化学习的约束流形策略框架（Constrained Manifold Policy Learning，CMPL），通过约束流形建模、可达性分析与状态估计的协同设计，实现策略安全性与可部署性的兼顾。整体框架由以下三个核心模块组成：
\begin{itemize}
...
为提升系统对环境约束的感知能力，本文引入 Hamilton--Jacobi 可达性分析方法，在离线阶段基于采样轨迹与环境先验信息预计算安全可达区域。所得可行性值函数在部署时用于动态调节安全裕度，使系统在无需实时精确建模障碍物的情况下，仍能保持保守且稳定的安全行为。
...
上述可达性分析与状态估计模块共同遵循“双离线预计算”范式：前者基于环境数据学习安全裕度分布，后者基于交互数据建模感知噪声特性。两者均可在离线阶段完成训练与构建，在部署时无需在线更新，从而显著降低计算开销，提高方法在资源受限平台上的可落地性。
...
此外，考虑到机器人系统（如差速驱动轮式机器人或无人机）普遍存在非霍洛诺姆约束（Nonholonomic Constraints），我们不能简单地假设状态可以任意改变。因此，我们将机器人的底层运动学建模为连续时间的控制仿射系统（Control-Affine System）：$$\dot{s} = f(s) + G(s) a$$
...
\section{实验}
\subsection{任务与环境}
为了验证所提出方法在不确定环境下安全导航任务中的有效性，本文围绕以下三个核心问题展开实验分析：
\begin{itemize}
    \item \textbf{安全可达空间构建：}
...
\end{figure*}
\subsection{对比实验}
\textbf{任务.}我们在 Safety-Gymnasium 基准环境中选取了三个单智能体任务以及一个多智能体任务进行评估：
\begin{itemize}
...
我们的方法（CMPL）在所有任务和惩罚设置下都取得了最低的代价（cost），表明该方法对惩罚参数调节具有很强的鲁棒性。相比之下，基线方法对惩罚系数非常敏感。例如，在 Goal 任务中，PPO-Lag 的 cost 从 5.46（$r_{\text{col}}=0.1$）变化到 13.91（$r_{\text{col}}=0.5$），波动达到 $2.5\times$。在 Circle 和 Push 任务中，基线方法的 cost 更高（约在 5–38 之间），而我们的方法始终保持接近零的 cost（Circle 为 1.38–1.55，Push 为 0.92–1.23）。当惩罚降低（$r_{\text{col}}=0.1$）时 cost 略有增加，这是因为强化学习策略在碰撞惩罚减小时会更加激进地探索，从而产生对安全过滤器更具挑战性的动作输入。

CMPL 的回合长度在不同 $r_{\text{col}}$ 下保持稳定（Goal 任务约为 350 步），因为安全过滤器会在靠近障碍物时施加速度限制，而这一机制与惩罚系数无关。相比之下，基线方法在较低惩罚下往往选择更冒险、更快速的路径，从而导致回合长度缩短。值得注意的是，在保持几乎零约束违反的同时，CMPL 仍然获得了具有竞争力的奖励（在 Goal 和 Push 任务上达到最高），并且是唯一一个始终保持接近零约束违规的方法。

在包含两个智能体的 Multi-Goal 任务中，由于智能体之间以及与障碍物之间的碰撞，所有基线方法都产生了较高的 cost（$12.48–36.13$）。相比之下，CMPL 将 cost 降低到接近零（$0.07$），同时仍保持正向奖励。这表明我们提出的约束流形安全过滤方法可以自然扩展到多智能体场景，在这种场景中，避碰不仅需要考虑静态障碍物，还需要考虑其他智能体。
\begin{table*}[h!]
    \begin{center}
...
采用基于策略梯度的近端策略优化（Proximal Policy Optimization, PPO）算法作为基础强化学习方法，通过在 PPO 基线方法上逐步加入各个模块，
...
本章围绕安全强化学习在机器人导航避障问题，构建了“安全可达空间刻画 + 策略生成 + 安全过滤 + 状态估计”的完整执行链路。
针对强化学习策略在复杂场景下可能出现的约束违规风险，本文首先从系统约束出发刻画了可用于在线决策的安全可达空间，
...
仿真实验结果表明：所提出方法在不同障碍密度和控制策略下均能有效降低碰撞风险，
为后续多机器人安全协同控制方法的系统扩展奠定了基础。
