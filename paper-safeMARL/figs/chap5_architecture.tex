...
\definecolor{cOursAlt}{HTML}{F57C00}   % 本文方法集成: 中橙 (Material Orange 700)
\definecolor{cOursEval}{HTML}{000000}  % 本文评估管线: 黑 (原 2CA02C 绿)
\definecolor{cUpstream}{HTML}{7F7F7F}  % 通用部件: 灰
\definecolor{cBgLayer}{HTML}{F0F0F0}
\definecolor{cBgHighlight}{HTML}{FFF3E0}  % L4 高亮底色: 浅暖橙 (Material Orange 50)
...
\draw[flow, rounded corners=8pt]
  ...
  node[right, pos=0.5, xshift=2pt, font=\tiny, fill=white, inner sep=1pt]{状态上行}
  ($(L4.west)+(-0.6cm,0)$);
\draw[flow, rounded corners=8pt]
  ($(L4.east)+(0.6cm,0)$)
    .. controls ($(L4.east)+(1.6cm,-1.0cm)$) and ($(L1.east)+(1.6cm,1.0cm)$) ..
  node[left, pos=0.5, xshift=-2pt, font=\tiny, fill=white, inner sep=1pt]{指令下行}
  ($(L1.east)+(0.6cm,0)$);
