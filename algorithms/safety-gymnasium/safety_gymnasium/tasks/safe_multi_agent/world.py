...
class World:  # pylint: disable=too-many-instance-attributes
    ...
    def build(self):  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
        ...
        if os.path.basename(str(self.agent_base)) == 'multi_point.xml':
            from safety_gymnasium.tasks.safe_multi_agent.assets.xmls.multi_point_dynamic import (
                multi_point_mjcf,
            )

            self.agent_base_xml = multi_point_mjcf(self._agent.nums)
        else:
        ...
        # Move each agent body to its starting pose
        agent_bodies = worldbody['body']
        if isinstance(agent_bodies, dict):
            agent_bodies = [agent_bodies]
        na = self._agent.nums
        agent_xy_flat = np.asarray(self.agent_xy, dtype=np.float64).reshape(-1)
        if agent_xy_flat.size < 2 * na:
            raise ValueError(
                f'agent_xy has {agent_xy_flat.size} coords but need {2 * na} for {na} agents',
            )
        rots = self.agent_rot
        if isinstance(rots, (int, float, np.floating)):
            rots = [float(rots)] * na
        elif len(rots) < na:
            raise ValueError('agent_rot list shorter than number of agents')
        for i in range(na):
            agent_bodies[i]['@pos'] = convert(
                np.r_[agent_xy_flat[2 * i : 2 * i + 2], self._agent.z_height],
            )
            agent_bodies[i]['@quat'] = convert(rot2quat(float(rots[i])))
        worldbody['body'] = agent_bodies
        ...
        # Build and add a tracking camera per agent body
        for bi in range(na):
            theta = float(rots[bi])
            xyaxes = {
                'x1': np.cos(theta),
            ...
            tname = 'track' if bi == 0 else f'track{bi}'
            cam_xml = (
                f'<b><camera name="{tname}" mode="track" pos="{pos["xp"]} {pos["yp"]} {pos["zp"]}" '
                f'xyaxes="{xyaxes["x1"]} {xyaxes["x2"]} {xyaxes["x3"]} '
                f'{xyaxes["y1"]} {xyaxes["y2"]} {xyaxes["y3"]}"/></b>'
            )
            track_cam = xmltodict.parse(cam_xml)
            bdy = agent_bodies[bi]
            tc = track_cam['b']['camera']
            if 'camera' in bdy:
                if isinstance(bdy['camera'], list):
                    bdy['camera'] = bdy['camera'] + [tc]
                else:
                    bdy['camera'] = [bdy['camera'], tc]
            else:
                bdy['camera'] = [tc]
