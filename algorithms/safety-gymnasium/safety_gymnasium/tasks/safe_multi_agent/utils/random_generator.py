...
class RandomGenerator:
    ...
    def sample_layout(self) -> bool:
        ...
        layout = {}
        if 'agent' in self.placements:
            agent_placements = self.placements['agent']
            agent_keepout = agent_placements[1]
        else:
            agent_placements = None
        ...
        if agent_placements is None:
            self.layout = layout
            return True

        agent_xy_list = []
        na = max(1, self.num_agents)

        def ok_vs_obstacles(xy, lay):
            for other_name, other_xy in lay.items():
                other_keepout = self.placements[other_name][1]
                dist = np.sqrt(np.sum(np.square(xy - other_xy)))
                if dist < other_keepout + self.placements_margin + agent_keepout:
                    return False
            return True
        ...
        for _ in range(na):
            ...
            for _try in range(100):
                xy = self.draw_placement(agent_placements[0], agent_placements[1])
                if not ok_vs_obstacles(xy, layout):
                    continue
                if any(
                    np.linalg.norm(xy - p) < 2 * agent_keepout + self.placements_margin
                    for p in agent_xy_list
                ):
                    continue
                agent_xy_list.append(xy)
                conflicted = False
                break
            if conflicted:
                return False

        layout['agent'] = agent_xy_list[0] if na == 1 else agent_xy_list
        self.layout = layout
        return True
        ...
        def iter_layout_xy():
            for other_name, other_xy in self.layout.items():
                if other_name == 'agent' and isinstance(other_xy, list):
                    for p in other_xy:
                        yield np.asarray(p), self.placements[other_name][1]
                else:
                    yield np.asarray(other_xy), self.placements[other_name][1]

        placements, keepout = self.placements['goal_red']
        goal_xy = self.draw_placement(placements, keepout)
        for other_xy, other_keepout in iter_layout_xy():
