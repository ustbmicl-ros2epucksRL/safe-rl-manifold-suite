...
class Underlying(abc.ABC):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, config: dict | None = None) -> None:
        ...
        # Formation navigation (MultiFormationGoal0; optional keys merged by Builder / Safe-PO)
        self.formation_shape: str = 'mesh'
        self.formation_line_axis: str = 'x'
        self.formation_wedge_half_angle_deg: float = 35.0
        self.formation_desired_direction: list = [0.0, 1.0]
        self.formation_target_distance: float | None = None
        self.formation_tolerance: float | None = None
        self.formation_reward_scale: float | None = None
        self.mesh_alignment_reward_scale: float | None = None
        self.line_width_reward_scale: float | None = None
        self._parse(config)
        self.agent = None
    ...
    def _parse(self, config: dict | None) -> None:
        """Parse a config dict.
    ...
    def _build_agent(self, agent_name: str) -> None:
        ...
        if agent_name.lower() == 'point':
            self.agent = agent_cls(random_generator=self.random_generator, num_agents=self.num_agents)
        else:
            if self.num_agents != 2:
                raise NotImplementedError(
                    f'Multi-agent XML for {agent_name} only supports num_agents=2; use Point for more.',
                )
    ...
    def simulation_forward(self, action: np.ndarray) -> None:
        ...
        if self.debug:
            ...
            nu_total = sum(bi.nu for bi in self.agent.body_info)
            noise = (
                self.action_noise * self.random_generator.randn(nu_total)
