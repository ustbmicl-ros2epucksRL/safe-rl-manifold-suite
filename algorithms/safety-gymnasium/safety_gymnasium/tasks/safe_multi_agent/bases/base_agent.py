...
class BaseAgent(abc.ABC):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        Args:
            ...
            num_agents (int): Number of robots in the multi-agent XML (Point only; others fixed at 2).
        """
        ...
        self.rot: float = rot
        self.nums: int = int(num_agents)
        if self.nums < 1:
            raise ValueError('num_agents must be >= 1')
        self.possible_agents: list = [f'agent_{i}' for i in range(self.nums)]

        self.engine: Engine = None
        self._load_model()
        self.sensor_conf = SensorConf()
        self.sensor_conf.sensors = self._default_imu_sensor_names()
        self.sensor_info = SensorInfo()
        self.body_info = [BodyInfo() for _ in range(self.nums)]
    ...
    def _default_imu_sensor_names(self) -> tuple:
        names = []
    ...
    @staticmethod
    def body_name_for_index(index: int) -> str:
        """MuJoCo body name for robot ``index`` (0-based)."""
        if index == 0:
            return 'agent'
        return f'agent{index}'

    def pos_at(self, index: int) -> np.ndarray:
        """Position (world) for robot ``index``."""
        return self.engine.data.body(self.body_name_for_index(index)).xpos.copy()

    def mat_at(self, index: int) -> np.ndarray:
        """Rotation matrix (3x3 flat) for robot ``index``."""
        return self.engine.data.body(self.body_name_for_index(index)).xmat.copy().reshape(3, -1)
    ...
    def _load_model(self) -> None:
        ...
        if os.path.basename(self.base) == 'multi_point.xml':
            from safety_gymnasium.tasks.safe_multi_agent.assets.xmls.multi_point_dynamic import (
                multi_point_mjcf,
            )

            model = mujoco.MjModel.from_xml_string(multi_point_mjcf(self.nums).encode())  # pylint: disable=no-member
        else:
            if self.nums != 2:
                raise ValueError(f'{self.base} only supports num_agents=2')
    ...
    def _init_body_info(self) -> None:
        ...
        if len(geom_names_all) != 2 * na:
            raise RuntimeError(
                f'Expected {2 * na} non-floor geoms for {na} agents, got {len(geom_names_all)}',
            )
        for i in range(na):
            self.body_info[i].nq = int(self.engine.model.nq / na)
            self.body_info[i].nv = int(self.engine.model.nv / na)
            self.body_info[i].nu = int(self.engine.model.nu / na)
            self.body_info[i].nbody = max(1, int(self.engine.model.nbody / na))
            self.body_info[i].geom_names = geom_names_all[i * 2 : (i + 1) * 2]
    ...
    def _build_action_space(self) -> gymnasium.spaces.Box:
        ...
        low, high = bounds.T
        per = int(len(low) / self.nums)
        return {
            f'agent_{i}': spaces.Box(
                low=low[i * per : (i + 1) * per],
                high=high[i * per : (i + 1) * per],
                dtype=np.float64,
            )
            for i in range(self.nums)
        }
    ...
    def build_sensor_observation_space(self) -> gymnasium.spaces.Dict:
        ...
        if self.sensor_info.freejoint_pos_name:
            ...
            for i in range(1, self.nums):
                obs_space_dict[sensor + str(i)] = gymnasium.spaces.Box(
                    -np.inf,
                    np.inf,
                    (1,),
                    dtype=np.float64,
    ...
    def obs_sensor(self) -> dict[str, np.ndarray]:
        ...
        if self.sensor_info.freejoint_pos_name:
            ...
            for i in range(1, self.nums):
                obs[sensor + str(i)] = self.get_sensor(sensor + str(i))[2:]
        if self.sensor_info.freejoint_qvel_name:
            sensor = self.sensor_info.freejoint_qvel_name
            obs[sensor] = self.get_sensor(sensor)
            for i in range(1, self.nums):
                obs[sensor + str(i)] = self.get_sensor(sensor + str(i))
