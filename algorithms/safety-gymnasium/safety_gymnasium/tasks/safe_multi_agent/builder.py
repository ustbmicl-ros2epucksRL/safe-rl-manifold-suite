...
class Builder(gymnasium.Env, gymnasium.utils.EzPickle):
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        formation_shape: str | None = None,
        formation_line_axis: str | None = None,
        formation_wedge_half_angle_deg: float | None = None,
        formation_desired_direction: list | tuple | None = None,
        ...
        Args:
            ...
            camera_id (int): Camera id to render.
            camera_name (str): Camera name to render.
            num_agents (int): When set, merged into ``config['num_agents']`` for multi-agent Point robots.
            formation_shape: Optional override for formation navigation tasks (``mesh`` | ``line`` | ``wedge`` | ``circle``).
            formation_line_axis: For ``line`` shape: ``x`` or ``y``.
            formation_wedge_half_angle_deg: For ``wedge`` shape, half opening angle at apex (degrees).
            formation_desired_direction: Length-2 vector (mesh edge preference / circle rotation); merged list into config.
            formation_target_distance: Target spacing for formation reward / constraints.
        """
        cfg = copy.deepcopy(config) if config is not None else {}
        if num_agents is not None:
            cfg['num_agents'] = int(num_agents)
        if formation_shape is not None:
            cfg['formation_shape'] = str(formation_shape).strip().lower()
        if formation_line_axis is not None:
            cfg['formation_line_axis'] = str(formation_line_axis).strip().lower()
        if formation_wedge_half_angle_deg is not None:
            cfg['formation_wedge_half_angle_deg'] = float(formation_wedge_half_angle_deg)
        if formation_desired_direction is not None:
            cfg['formation_desired_direction'] = list(formation_desired_direction)
        if formation_target_distance is not None:
            cfg['formation_target_distance'] = float(formation_target_distance)
        gymnasium.utils.EzPickle.__init__(self, config=cfg)
    ...
    def reset(
        ...
        *,
        ...
        for ag in self.possible_agents:
            assert cost[ag]['cost_sum'] == 0, f'World has starting cost! {cost}'
    ...
    def step(self, action: dict) -> tuple[np.ndarray, float, float, bool, bool, dict]:
        ...
        if exception:
            ...
            costs = {ag: info[ag]['cost_sum'] for ag in self.possible_agents}
    ...
    def _reward(self) -> float:
        ...
        if reward_clip:
            for agent_name, reward_i in reward.items():
                in_range = -reward_clip < reward_i < reward_clip
                if not in_range:
                    reward[agent_name] = np.clip(reward_i, -reward_clip, reward_clip)
