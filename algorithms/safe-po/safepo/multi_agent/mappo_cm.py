...
from safepo.common.env import make_ma_mujoco_env, make_ma_isaac_env, make_ma_multi_goal_env, make_cm_multi_goal_env
...
class Runner:
    ...
    def run(self):
        ...
        # Nav_cm = MultiNavAtacom(base_env=self.envs, timestep=0.002)
        # Nav_cm.reset()
...
def train(args, cfg_train):
    ...
    if args.task in multi_agent_velocity_map:
        ...
        env = make_cm_multi_goal_env(task=args.task, seed=args.seed, cfg_train=cfg_train)
        cfg_eval = copy.deepcopy(cfg_train)
        cfg_eval["seed"] = args.seed + 10000
        cfg_eval["n_rollout_threads"] = cfg_eval["n_eval_rollout_threads"]
        eval_env = make_cm_multi_goal_env(task=args.task, seed=args.seed + 10000, cfg_train=cfg_eval)
    else: 
