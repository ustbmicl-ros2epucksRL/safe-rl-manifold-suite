...
class MultiAgentActorObsOnlyTraceWrapper(nn.Module):
    """Wrap ``MultiAgentActor`` for ``torch.jit.trace``: ``obs -> actions`` (deterministic mean)."""

    def __init__(self, ma_actor: "MultiAgentActor"):
        super().__init__()
        self.ma_actor = ma_actor
        self._recurrent_n = int(ma_actor.config["recurrent_N"])
        self._hidden_size = int(ma_actor.hidden_size)
    ...
    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        masks = torch.ones(batch, 1, device=obs.device, dtype=obs.dtype)
        actions, _, _ = self.ma_actor(obs, rnn_states, masks, None, True)
        return actions


def save_multi_agent_actor_traced_jit(
    ...
    """Export actor with ``torch.jit.trace`` to ``jit_path`` (CPU); no-op if ``save_jit_actor`` is false."""
    if not config.get("save_jit_actor", True):
        return
    actor.eval()
    try:
        obs_shape = get_shape_from_obs_space(obs_space)
        obs_dim = int(obs_shape[0])
        example = torch.zeros(1, obs_dim, device=device, dtype=torch.float32)
        wrapped = MultiAgentActorObsOnlyTraceWrapper(actor).eval()
        traced = torch.jit.trace(wrapped, example, strict=False)
        torch.jit.save(traced.cpu(), jit_path)
    except Exception as e:
        print(
            f"Warning: torch.jit.trace save failed for agent {agent_id} "
            f"(state_dict still saved): {e}"
        )
